 ...
 *
 * Delivers Web Push notifications to a specific client's subscribed device.
 * Called by other Edge Functions (auto-remind) or from the coach app (app.html)
 * when sending messages or updating programs.
 *
 ...
 * Payload format:
 *   {
 *     clientId: "xyz456",                // required — lookup key in client_push_subs
 *     title: "New Message",
 *     body: "Coach Mike sent you a message",
 *     type: "message" | "program_update" | "session_reminder" | "general",
 *     tag: "msg-xyz456",                 // optional, deduplicates
 *     url: "./portal.html",              // optional, deep link on click
...
serve(async (req) => {
  ...
  try {
    ...
    const { clientId, title, body, type, tag, url, requireInteraction } = payload;
    ...
    // Fetch push subscription from client_push_subs
    const { data: subRow, error: subErr } = await supabase
      .from("client_push_subs")
      .select("data")
      .eq("id", clientId)
      .single();

    if (subErr || !subRow?.data?.subscription) {
      return new Response(
    ...
    if (subRow.data.muted) {
      return new Response(JSON.stringify({ skipped: "muted" }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const subscription = subRow.data.subscription;
    ...
    if (pushRes.status === 410 || pushRes.status === 404) {
      ...
      await supabase.from("client_push_subs").delete().eq("id", clientId);
      return new Response(
        JSON.stringify({ error: "Push subscription expired", expired: true }),
